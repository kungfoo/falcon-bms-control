...
function async:new()
  return self:init({tasks = {}})
end

-- add a task to the kernel
function async:call(f, args, cb, error_cb)
  table.insert(self.tasks, {coroutine.create(f), args, cb, error_cb})
end
...
function async:update()
  ...
  if args then a, b, c, d, e, f, g, h = unpack(args) end
...
function async:update_for_time(t)
  ...
  while love.timer.getTime() - now < t do if not self:update() then break end end
end

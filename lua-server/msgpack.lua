...
local function checktype(caller, narg, arg, tname)
  if type(arg) ~= tname then typeerror(caller, narg, arg, tname) end
end
...
packers['string_compat'] = function(buffer, str)
  ...
  elseif n <= 0xFFFF then
    ...
    floor(n / 0x100), n % 0x100)
  elseif n <= 4294967295.0 then
    buffer[#buffer + 1] = char(0xDB, -- str32
    floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
  else
...
packers['unsigned'] = function(buffer, n)
  if n >= 0 then
    ...
    elseif n <= 4294967295.0 then
      ...
      floor(n / 0x1000000000000) % 0x100, floor(n / 0x10000000000) % 0x100, floor(n / 0x100000000) % 0x100,
                                 floor(n / 0x1000000) % 0x100, floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100,
                                 n % 0x100)
    end
  else
    if n >= -0x20 then
...
packers['float'] = function(buffer, n)
  ...
  elseif (mant == 0.0 and expo == 0) or expo < -0x7E then
    ...
    buffer[#buffer + 1] = char(0xCA, sign + floor(expo / 0x2), (expo % 0x2) * 0x80 + floor(mant / 0x10000),
...
packers['double'] = function(buffer, n)
  ...
  elseif (mant == 0.0 and expo == 0) or expo < -0x3FE then
    ...
    buffer[#buffer + 1] = char(0xCB, sign + floor(expo / 0x10), (expo % 0x10) * 0x10 + floor(mant / 0x1000000000000),
                               floor(mant / 0x10000000000) % 0x100, floor(mant / 0x100000000) % 0x100,
                               floor(mant / 0x1000000) % 0x100, floor(mant / 0x10000) % 0x100,
                               floor(mant / 0x100) % 0x100, mant % 0x100)
  end
end
...
packers['ext'] = function(buffer, tag, data)
  ...
  if n <= 0xFF then
    ...
    n, tag < 0 and tag + 0x100 or tag)
  elseif n <= 0xFFFF then
    buffer[#buffer + 1] = char(0xC8, -- ext16
    floor(n / 0x100), n % 0x100, tag < 0 and tag + 0x100 or tag)
  elseif n <= 4294967295.0 then
...
local function unpack_array(c, n)
  local t = {}
  for i = 1, n do t[i] = unpack_cursor(c) end
  return t
end
...
local function unpack_map(c, n)
  ...
  for i = 1, n do
    ...
    if k == nil or k ~= k then k = m.sentinel end
...
local function unpack_int64(c)
  ...
  if b1 < 0x80 then
    ...
    return
      ((((((((b1 - 0xFF) * 0x100 + (b2 - 0xFF)) * 0x100 + (b3 - 0xFF)) * 0x100 + (b4 - 0xFF)) * 0x100 + (b5 - 0xFF)) *
        0x100 + (b6 - 0xFF)) * 0x100 + (b7 - 0xFF)) * 0x100 + (b8 - 0xFF)) - 1
  end
end
...
local function cursor_loader(ld)
  return {
    ...
    underflow = function(self, e)
      ...
      while e > self.j do
        local chunk = ld()
        if not chunk then error "missing bytes" end
        self.s = self.s .. chunk
        self.j = #self.s
      end
    end,
...
function m.unpack(s)
  ...
  if cursor.i <= cursor.j then error "extra bytes" end
  return data
end
...
function m.unpacker(src)
  ...
  elseif type(src) == 'function' then
    ...
    return function()
      if cursor.i > cursor.j then pcall(cursor.underflow, cursor, cursor.i) end
...
if SIZEOF_NUMBER == 4 then
  ...
  mininteger = -maxinteger
  set_number 'double'
  if SIZEOF_NUMBER > 8 then m.long_double = true end
end

...
local function argerror(caller, narg, extramsg)
    error("bad argument #" .. tostring(narg) .. " to " .. caller .. " (" ..
              extramsg .. ")")
end
...
packers['string_compat'] = function(buffer, str)
    ...
    elseif n <= 4294967295.0 then
        ...
        floor(n / 0x1000000), floor(n / 0x10000) % 0x100,
...
packers['table'] = function(buffer, tbl) packers['_table'](buffer, tbl) end
...
packers['float'] = function(buffer, n)
    ...
    elseif (mant == 0.0 and expo == 0) or expo < -0x7E then
        ...
        buffer[#buffer + 1] = char(0xCA, sign + floor(expo / 0x2),
...
packers['double'] = function(buffer, n)
    ...
    elseif (mant == 0.0 and expo == 0) or expo < -0x3FE then
        ...
        buffer[#buffer + 1] = char(0xCB, sign + floor(expo / 0x10),
                                   (expo % 0x10) * 0x10 +
                                       floor(mant / 0x1000000000000),
...
local function unpack_double(c)
    ...
    local mant = ((((((b2 % 0x10) * 0x100 + b3) * 0x100 + b4) * 0x100 + b5) *
...
local function unpack_int64(c)
    ...
    if b1 < 0x80 then
        return
            ((((((b1 * 0x100 + b2) * 0x100 + b3) * 0x100 + b4) * 0x100 + b5) *
                0x100 + b6) * 0x100 + b7) * 0x100 + b8
    else
        return
            ((((((((b1 - 0xFF) * 0x100 + (b2 - 0xFF)) * 0x100 + (b3 - 0xFF)) *
                0x100 + (b4 - 0xFF)) * 0x100 + (b5 - 0xFF)) * 0x100 +
                (b6 - 0xFF)) * 0x100 + (b7 - 0xFF)) * 0x100 + (b8 - 0xFF)) - 1
    end
end

function m.build_ext(tag, data) return nil end

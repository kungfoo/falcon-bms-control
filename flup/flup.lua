...
function flup.internal.updateGeometry(node, x, y, w, h)
  if node.direction and node.ratio and node.components then
    ...
    if node.direction == "x" then
      ...
      if node.components.left then flup.internal.updateGeometry(node.components.left, x, y, p, h) end

      if node.components.right then flup.internal.updateGeometry(node.components.right, x + p + 1, y, w - p, h) end
    end

    if node.direction == "y" then
      local p = h * node.ratio
      if node.components.top then flup.internal.updateGeometry(node.components.top, x, y, w, p) end

      if node.components.bottom then flup.internal.updateGeometry(node.components.bottom, x, y + p + 1, w, h - p) end
    end
  else
